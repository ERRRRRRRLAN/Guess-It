...
// UI Elements
const pages = ['page-menu', 'page-difficulty', 'page-game', 'page-result', 'page-leaderboard', 'page-login'];
const inputField = document.getElementById('guess-input');
const backBtn = document.getElementById('back-btn');

function showPage(pageId, isBack = false, isPopState = false) {
    pages.forEach(id => {
...
window.onpopstate = (e) => {
    ...
    showPage(pageId, true, true);
};

function goBack() {
    // Manual back button trigger
    history.back();
}
    ...
    if (isWin) {
        ...
        extraInfo = `
            ...
            <div class="name-entry-area">
                <input type="text" id="player-name" placeholder="MASUKKAN NAMA" maxlength="10">
...
// Global Button Sound (Hover)
document.addEventListener('mouseover', (e) => {
    if (e.target.tagName === 'BUTTON' || e.target.closest('button') || e.target.classList.contains('diff-btn')) {
        playSound('select');
    }
});
